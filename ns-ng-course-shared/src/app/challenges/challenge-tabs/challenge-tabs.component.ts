...
export class ChallengeTabsComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.page.on('navigatedTo', (data) => {
            ...
            if (!data['object']['_onUnloadedCalled']) {
                 return;
            }
            const bottomNav = this.bottomNavEl.nativeElement;
            const currentSelect = bottomNav.selectedIndex;
            const index = currentSelect === bottomNav.items.length - 1 ? currentSelect - 1 : currentSelect + 1;
            if (index < 0) {
                return;
            }
